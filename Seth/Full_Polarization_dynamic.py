...
class Calculation:
    ...
    def __init__(self, cif_file):
        ...
        self.quadrupole_tensor = None
        self.diagonalized_quadrupole_tensor = None
        self.zero_quadrupole_tensor = None
        self.close_to_zero_quadrupole_tensor = None
    ...
    def write_metadata_to_json(self, output_directory: Path):
        ...
        metadata = {
            ...
            'quadrupole tensor': self.quadrupole_tensor,
            'diagonalized quadrupole tensor': self.diagonalized_quadrupole_tensor,
            'zero quadrupole': self.zero_quadrupole_tensor,
            'close to zero quadrupole': self.close_to_zero_quadrupole_tensor
        }
...
def get_oxidation_state_formula(cif_file:Path) -> dict:
    ...
    try:
        ...
        if composition.num_atoms > 40:
            #print(f"[SKIPPING] Reduced formula {reduced_formula} has {composition.num_atoms} atoms (>40), skipping oxidation state guessing.")
            return "Skipping"
...
def diagonalize_matrix(matrix):
    ...
    return np.real(eigenvalues), np.real(eigenvectors), np.real(D_clean)
...
def write_qsub_script(instance):
    ...
    with open(script_path, "w") as script_file:
        ...
        script_file.write("#PBS -l nodes=1:ppn=40\n")
...
def wait_for_file(file_path, timeout=120000, check_interval=5):
    """
...
def main(TARGET_DIRECTORY, ABSORBING_ATOM):
    ...
    for cif_file in CIF_PATHS:
        ...
        oxidation_dict = get_oxidation_state_formula(calc_instance.cif_file)
        if isinstance(oxidation_dict, str):
           # print("[SKIPPING] oxidation state cannot be determined: ", oxidation_dict)
           # print(f"I am breaking the for loop because I am {oxidation_dict} this is for {cif_file}")
            oxi_fail += 1
            continue
        
        calc_instance.oxidation_dict = oxidation_dict
        oxi_succ += 1

        shutil.copy(src=cif_file, dst=calc_directory)
       # print(f"Copied {cif_file} to {calc_directory}")
        all_calculation_instances.append(calc_instance)

    print(f"\nTotal materials: {oxi_succ + oxi_fail}")
    print(f"Total failed oxidation step: {oxi_fail}")
    print(f"Total succeeded oxidation step: {oxi_succ}\n")
    ...
    print("Step 1 is now finished!")

    good_calculation_list = []

    for instance in all_calculation_instances:
        #print(instance.cif_file.stem)
        min_radius = 0
        min_atoms = 30
        ...
        oxidation_dict = instance.oxidation_dict
        ...
        try:
            ...
            #print("[SKIPPPING]", individual_path)
            print("Error: ", e)
            continue

        failures = [len(cluster_array) < min_atoms for cluster_array in list_of_cluster_arrays]
        if any(failures):
            print("[SKIPPPING]", individual_path)
            print("Cluster has abnormally small number of atoms, probably unphysical")
            continue

        for cluster_array in list_of_cluster_arrays:
            oxi, x, y, z = cluster_array[min_atoms - 1]
            radius = np.sqrt(x**2 + y**2 + z**2)
            if radius > min_radius:
                min_radius = radius
        min_radius = np.round(min_radius * 1.05, 2)

        list_of_quadrupole_matrices = quadrupole_moment(list_of_cluster_arrays)
        #print(list_of_quadrupole_matrices)
        avg_quad_matrix = average_over_all_quadrupole_moments(list_of_quadrupole_matrices)
        #print(avg_quad_matrix)
        if np.any(avg_quad_matrix == 0):
#We set a status of true in the metadata of the material
#if the quad tensor is zero
#We can then query later for analysis
            instance.zero_quadrupole_tensor = True
#here we sort out the zero quadrupole moments
            print(f"[SKIPPING] completely zero matrix, no need to run. This is the job {individual_path}")
            continue

#Now we see if there are close to zero quad matrices 
#and do the same thing
        elif np.allclose(avg_quad_matrix, 0):
            instsance.close_to_zero_quadrupole_tensor = True

        eigvalues, eigvecs, D_clean = diagonalize_matrix(avg_quad_matrix.copy())

        eigvecs = eigvecs.T # For writing to file simplicty
        instance.quadrupole_tensor = avg_quad_matrix.tolist()
        instance.diagonalized_quadrupole_tensor = D_clean.tolist()

       # print("This is supposed to be the diagonalized tensor: ", instance.diagonalized_quadrupole_tensor)


#Now that we have sorted out all of the initial conditions, non-convergences,
#and math on the quadrupole matrices, we now reupdate the job submission script
#to now calculate the linear polarizations with Corvus. We now take the
#eigen vectors and make them the linear polarization direction 
# that maximizes the linear anisotropy of the system
        ...
        instance.update_input_file('polarization', value=matrix_turned_into_string)
        instance.update_input_file('feff.scf', value=f'{min_radius} 0 100 0.05 0')
        instance.update_input_file('feff.fms', value=f'{min_radius} 0 0 0.0 0.0 40')
        instance.write_corvus_in_file(individual_path)

        #we now write to the hidden metadata file for future bookeeping
        instance.write_metadata_to_json(individual_path)

        json_dict = {
            'Determined Charges': {Element(el).Z: oxi for el, oxi in oxidation_dict.items()},
            'Avg Quadrupole Matrix': instance.quadrupole_tensor,
            'Avg Diagonalized Quadrupole matrix': instance.diagonalized_quadrupole_tensor
            }            
    ...
    good_input_list_file = qsub.save_input_file_list(good_calculation_list, "corvus_input_paths.txt")
